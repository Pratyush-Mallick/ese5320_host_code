...
XCLBIN := hardware_encoding.xclbin
ALL_MESSAGE_FILES = $(subst .xo,.mdb,$(XO)) $(subst .xclbin,.mdb,$(XCLBIN))

#
# host files
#
HOST_SOURCES = Host.cpp cdc.cpp sha256.cpp decoder.cpp dedup.cpp Utilities.cpp server.cpp main.h lzw.cpp
HOST_OBJECTS =$(HOST_SOURCES:.cpp=.o)
HOST_EXE = host_fpga

CPU_SOURCES = Host.cpp
...
$(XO): lzw.cpp
	-@$(RM) $@
	$(VPP) $(VPP_OPTS) -k hardware_encoding --compile -I"$(<D)" --config design.cfg -o"$@" "$<"
